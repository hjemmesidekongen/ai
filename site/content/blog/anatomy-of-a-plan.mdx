...
flowchart TD
    ...
    INIT --> GATE{Goal clarity gate<br/>6-point checklist}
    GATE -->|"< 4/6 pass"| BLOCK[Block: refine goal]
    BLOCK --> GATE
    GATE -->|"≥ 4/6 pass"| ORIENT
...
acceptance_criteria:
  - criterion: "skill-reviewer passes plan-engine with zero criticals"
    verification:
      type: command
      value: "run skill-reviewer on plan-engine, check output for zero critical findings"
  - criterion: "agent-reviewer passes both agents with zero criticals"
    verification:
      type: command
      value: "run agent-reviewer on plan-verifier and plan-classifier"
  - criterion: "A/B trigger testing confirms 10/10 prompt coverage"
    verification:
      type: metric
      value: "trigger test results show 10/10 matches"
```

Each criterion has to be something you can check mechanically. "Works well" doesn't cut it. "Zero criticals on review" does. The `verification` block makes this explicit — every criterion declares how to check it and what the expected result looks like. Plans also declare a `done_signal` that tells the system how to detect completion automatically, so autopilot can terminate without human confirmation when all criteria are met.
...
So the system runs a mechanical checklist before cycle 1 starts. Six checks, no judgment calls:
...
If four or more checks pass, the plan proceeds (with warnings for failures). If fewer than four pass, it blocks and asks you to refine. This catches obviously bad goals without being so strict that it rejects reasonable ones.
...
```
Cycle 1:
  Orient  -> scan codebase, run goal clarity gate (6/6 pass)
