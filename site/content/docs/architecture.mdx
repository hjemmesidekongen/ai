...
herold writes task files. smedjen reads them (via `taskflow-bridge`). smedjen writes its own state to `.ai/smedjen/`. Neither plugin writes to the other's directories.
