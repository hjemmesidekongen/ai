...
The hjemmesidekongen/ai methodology is implemented as six plugins with one foundation. Each plugin has a clear responsibility boundary, communicates through data files, and never imports another plugin's skills directly. This architecture reflects the methodology's core constraint: agents work, but within strict boundaries.
...
├── skjalden         ← content production, depends on kronen
...
- Brand-to-content pipeline: våbenskjold writes `.ai/brand/` → skjalden reads it for brand-aware content. Brand-to-design pipeline: våbenskjold → segl → smedjen's design-to-code patterns. Each step is independent.
