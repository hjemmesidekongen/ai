...
| `/plan` | Create and run an iterative plan from a goal. Validates the goal with a 6-point mechanical checklist, plans one wave at a time, executes via multi-agent dispatch, verifies with an isolated agent, and loops until the goal is met. |
...
- **plan-verification-gate** blocks writes to future wave files before current wave verification passes, and blocks writes to files matching `scope.paths.exclude` patterns
- **plan-scope-guard** warns when writing outside declared task files or plan-level `scope.paths.include` patterns — advisory, not blocking
