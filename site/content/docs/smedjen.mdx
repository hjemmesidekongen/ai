...
**Version:** 0.2.0 · **62 skills** · **6 agents** · **2 commands**
...
|---------|-------------|
| `/smedjen:run` | Full pipeline - decompose, assign, dispatch, verify. Supports `--dry-run` for preview. |
| `/smedjen:scan` | Scan repo for tech stack and architecture. Outputs `.ai/project-map.yml` with Mermaid C4 diagram. |

Run the full pipeline:

```bash
/smedjen:run "Build user profile page with avatar upload and form validation"
```

Or preview without executing:

```bash
/smedjen:run "Build user profile page" --dry-run
```
...
└──────┬──────┘
       ...
       ▼
┌──────────────┐
│ taskflow-bridge │  Loads acceptance criteria (if herold active)
└──────┬───────┘
       │
       ▼
┌───────────────┐
...
- [våbenskjold](/docs/vaabenskjold) - brand strategy — content-writer agent consumes brand voice
