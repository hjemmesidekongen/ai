...
If hooks still do not fire after restarting, check the hook error log:
...
- **Script not executable.** Run `chmod +x plugins/kronen/scripts/<script>.sh`.
...
There is no downside to clearing cache. It rebuilds automatically on the next Claude Code startup.
...
export CLAUDE_PLUGIN_ROOT="plugins/kronen"
bash plugins/kronen/scripts/assemble-context.sh manual
