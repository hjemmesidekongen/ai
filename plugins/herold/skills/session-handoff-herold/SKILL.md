---
name: session-handoff-herold
description: >
  Save active task context for cross-session continuity — captures task state,
  subtask progress, blockers, and open questions. Extends kronen session-handoff
  with herold-specific fields. Detects prior handoffs on resume.
user_invocable: true
interactive: false
model_tier: senior
depends_on: []
# Complements kronen:session-handoff — operates independently, not a call chain
triggers:
  - "handoff"
  - "save progress"
  - "session end"
...
_source:
  origin: "herold"
  inspired_by: "kronen session-handoff"
  ported_date: "2026-03-10"
  iteration: 1
  changes: "New skill. Taskflow-specific handoff layer on top of kronen session-handoff."
---

# session-handoff-herold

Saves active task context before ending a session. Extends the kronen
`session-handoff` pattern with herold-specific fields: active task, subtask progress,
...
2. **Classify staleness** — same thresholds as kronen session-handoff (see process.md)
