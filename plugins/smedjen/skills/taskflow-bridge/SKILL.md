...
description: >
  Bridge between smedjen and taskflow. Uses active task context to enrich
  completion gate checks and map acceptance criteria to verifiable checks.
  If taskflow plugin is not installed, smedjen operates standalone without
  task context.
...
Connects smedjen's execution pipeline to taskflow's task context. When taskflow is installed, active task acceptance criteria drive the completion gate. When it is not, smedjen runs in standalone mode using only the agent-provided criteria.
...
**If taskflow plugin is not installed, smedjen operates standalone without task context.**
