...
Apply the 10-point criteria defined in the `completion-gate` skill (`plugins/smedjen/skills/completion-gate/SKILL.md`). That skill is the single source of truth for what counts as passing. A piece of work passes only when all applicable criteria are met.
