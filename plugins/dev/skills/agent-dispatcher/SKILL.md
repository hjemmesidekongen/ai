...
checkpoint:
  ...
  on_pass: "Update team-state.yml status, hand off to completion-gate."
---

# Agent Dispatcher

Phase 3 of /dev:build. Reads the wave plan, dispatches specialist agents via Task() with model tier mapping, manages parallel vs sequential waves, and collects structured reports.

## Context

| Aspect | Details |
|--------|---------|
| **Reads** | team-state.yml wave_plan, dev-config.yml, knowledge files (tag-filtered) |
| **Writes** | team-state.yml execution section (dispatched_tasks, commit_range) |
...
1. Read wave_plan from team-state.yml — get waves, tasks, assignments, tiers
2. For each wave in sequence:
   ...
   h. Update team-state.yml execution section with dispatch records
