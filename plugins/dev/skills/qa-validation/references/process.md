...
The QA validator is Phase 4b of /dev:build. It validates the entire build against
...
```
Read team-state.yml → decomposition (components, boundaries, files_affected)
Read team-state.yml → execution.dispatched_tasks (all reports)
Read team-state.yml → review.code_review (findings, status)
Read team-state.yml → build.feature_description (original user intent)
Read dev-config.yml → project context (framework, structure)
Read build-report-template.md → report format
```
...
Use `plugins/dev/resources/templates/build-report-template.md` to generate the
build report. Fill in all template variables from team-state.yml data:
...
7. **Blockers Encountered** — from team-state.yml errors array (task_id, reason, resolution)
8. **Commit Range** — base_sha, head_sha, total commit count

### Report Generation

```
1. Read build-report-template.md
2. For each section, extract data from team-state.yml
...
1. Stage only team-state.yml, build-report.md, and findings.md updates
...
- team-state.yml review.qa_validation section exists and is non-empty
...
- build-report.md exists and has at least 8 markdown headings (## sections)
...
1. **Missing decomposition:** If team-state.yml has no decomposition section,
...
3. **Build report template missing:** If build-report-template.md is not found,
   generate a minimal report with available data. Log the missing template
   to errors array.
