# session-handoff-taskflow — Process Detail

## Relationship to claude-core session-handoff

The claude-core `session-handoff` skill handles general work context: plans, recent edits,
decisions, branch state. This skill adds a taskflow-specific layer on top:

| Layer | Written by | Content |
|-------|-----------|---------|
| General handoff | claude-core session-handoff | Plans, files, decisions, branch state |
| Task handoff | session-handoff-taskflow | Active task, subtasks, blockers, open questions |
...
3. Classify staleness (same thresholds as claude-core session-handoff):
...
4. Most recent `.ai/handoffs/*.md` — general narrative context (from claude-core)
