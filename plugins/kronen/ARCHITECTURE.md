...
Kronen is the foundation plugin. It provides capabilities that all other
...
```
kronen          ← always installed, foundation for everything
claude-toolkit       ← opt-in, builder tools (skill/plugin/command creation)
development          ← domain plugin, depends only on kronen
marketing            ← domain plugin, depends only on kronen
devops               ← domain plugin, depends only on kronen
```
...
kronen captures what happened (every tool invocation), agency captures why it
...
**Agent directory:** `plugins/kronen/agents/`
