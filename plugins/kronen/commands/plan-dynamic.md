...
8. **Present and hand off:**
   ...
   Execute autonomously:
     /kronen:autopilot-run --dynamic-plan .ai/plans/{name} "Execute the dynamic plan"
...
**Autonomous execution:** Start with `/kronen:autopilot-run --dynamic-plan .ai/plans/{name}`. The stop hook calls `dynamic-prompt-constructor.sh` which reads state.yml and learnings.yml to build a cycle-aware prompt for each iteration. Stops automatically when the plan reaches `status: done` or `max_cycles`.
