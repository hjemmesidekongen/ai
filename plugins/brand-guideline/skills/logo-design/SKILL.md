...
description: >
  ...
  Produces all logo SVG variants, brand icon variants, clear space rules, misuse
  examples, and an interactive HTML preview page. Writes the visual.logo section
  and assets.logo / assets.brand_icon sections to brand-reference.yml.
phase: 6
...
checkpoint:
  ...
  required_checks:
    - name: "logo_svg_variants"
      verify: "All 6 logo SVG variants exist in assets/logo/svg/"
      files:
      ...
      fail_action: "Generate the missing SVG variants from the finalized design"
    - name: "brand_icon_variants"
      verify: "All 4 brand icon SVG variants exist in assets/brand-icon/"
      files:
      ...
      fail_action: "Derive brand icon variants from the finalized logo mark"
    - name: "svg_validity"
      verify: "All SVGs are valid XML with viewBox attribute, no fixed width/height"
      fail_action: "Fix invalid SVGs — ensure viewBox is set and remove fixed dimensions"
    - name: "svg_file_size"
      verify: "No SVG file exceeds 15KB"
      fail_action: "Optimize oversized SVGs — simplify paths, remove metadata, reduce precision"
    - name: "preview_html"
      verify: "preview/brand-preview.html exists and contains no broken references"
      fail_action: "Regenerate the preview page with correct paths to all SVG assets"
    - name: "logo_mark_small_size"
      verify: "logo-mark.svg is recognizable at 32px"
      type: manual_approval
      fail_action: "Simplify the logo mark — reduce detail until it reads clearly at 32px"
...
Phase 6 of brand generation. This is the most complex skill in the pipeline. It takes all brand data from Phases 1-5 and produces a complete logo system through a structured 5-phase design process.

This skill produces the logo — the visual identity skill (Phase 5) already defined the visual rules (imagery, iconography, shape language, layout) that the logo must follow.
