...
checkpoint:
  ...
  required_checks:
    - name: "schema_valid"
      verify: "brand-reference.yml validates against the full schema — all required sections present (meta, identity, visual, typography, colors, voice, audience, content, social)"
      fail_action: "List missing sections and re-run the corresponding skill to fill them"
    - name: "cross_references"
      verify: "All cross-references resolve: colors used in logos exist in palette, fonts in visual identity exist in typography, logo variants referenced in social exist in assets"
      fail_action: "List broken references with source → target and ask which correction to apply"
    - name: "manual_sections"
      verify: "brand-manual.md has all 9 sections: About the Brand, Target Audience, Brand Voice, Color System, Typography, Logo Usage, Visual Identity, Content Rules, Social Media"
      fail_action: "Identify missing sections and generate them from brand-reference.yml"
    - name: "docx_valid"
      verify: "brand-manual.docx is a valid Word file with heading hierarchy, table of contents, and page numbers"
      fail_action: "Regenerate the docx from brand-manual.md"
    - name: "script_executable"
      verify: "scripts/generate-assets.sh exists and is executable (chmod +x)"
      fail_action: "Create the script and set executable permission"
    - name: "webmanifest_valid"
      verify: "assets/favicons/site.webmanifest is valid JSON with name, icons array, and theme_color"
      fail_action: "Regenerate site.webmanifest from brand-reference.yml data"
    - name: "preview_html"
      verify: "preview/brand-preview.html exists and contains references to brand colors, typography, and logo assets"
      fail_action: "Generate the preview page from brand-reference.yml data"
  qa_review: mandatory
  on_fail: "Fix issues and re-run checkpoint. Do NOT mark brand generation as complete."
  on_pass: "Update state.yml → mark phase 9 complete. Trigger QA agent review of all outputs."
---
