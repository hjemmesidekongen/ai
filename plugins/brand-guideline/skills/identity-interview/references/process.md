# Identity Interview — Detailed Process

This skill runs as Phase 1 of brand generation. It collects all foundational brand data through
a conversational interview and writes the `meta` and `identity` sections of `brand-reference.yml`.
No prior brand data exists — this is the starting point for the entire brand pipeline.
