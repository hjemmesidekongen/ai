...
description: >
  ...
  skills (spec-generator, scaffolder) consume.
interactive: true
depends_on: []
reads:
...
Interactive skill that walks the user through the 8 design questions from `docs/ecosystem-strategy.md`. Every new plugin must complete this interview before any code or specs are written.
...
- `packages/[plugin-name]/design.yml` — the structured design document
...
These rules are enforced during the interview:
