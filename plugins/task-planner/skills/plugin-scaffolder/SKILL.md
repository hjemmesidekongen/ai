...
2. Create directory structure — .claude-plugin/, commands/, skills/ (with per-skill subdirs using progressive disclosure: lean SKILL.md + references/ for complex skills, stubs use positive framing in instruction text), resources/templates/, resources/examples/, scripts/; agents/ only if design.yml defines agents
