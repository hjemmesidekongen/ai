...
description: >
  Creates the physical directory structure, plugin.json manifest, and README.md
  for a new plugin based on design.yml. Produces the empty scaffold that
  downstream build steps (schema, skills, commands) populate.
interactive: false
depends_on:
...
Autonomous skill that reads the approved `design.yml` and creates the complete directory structure for a new plugin. This is the first physical artifact — everything before this (design, specs, execution guide) is documentation. After this skill runs, the plugin exists on disk and is ready for schema creation and skill implementation.
...
These rules are enforced during scaffolding:
