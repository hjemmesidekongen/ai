...
An open-ended brainstorm skill where Claude acts as a thinking partner. This is
NOT an interview. There are no required questions, no predefined phases, and no
structured output until the user explicitly calls `/brainstorm:decide`.

## Prerequisites

None. A brainstorm can happen before any plugin exists — it's often the
first step before `/plugin:create`.
