...
description: >
  ...
  (execution-guide-generator, scaffolder) need to build the plugin.
interactive: false
depends_on:
...
Autonomous skill that reads the `design.yml` produced by the plugin-design-interview and generates the complete spec documents for the new plugin. These specs are consumed by downstream skills (execution-guide-generator, scaffolder) and by humans building the plugin.
...
List all skills in the order they should be implemented (which may differ from execution order):
...
| # | Skill | Rationale |
|---|-------|-----------|
| 1 | [skill-name] | [Why build this first — e.g., "foundational data model, needed by all others"] |
| 2 | [skill-name] | [Why next — e.g., "depends only on skill 1, defines core schema sections"] |
| ... | ... | ... |
```
...
- `docs/[plugin-name]-addendum.md` — domain knowledge, quality standards, tools, validation criteria
- `docs/[plugin-name]-asset-manifest.md` — complete file list with generation methods (only if assets defined)
- `packages/task-planner/resources/verification-registry.yml` — updated with new types and profile
...
These rules are enforced during spec generation:
