...
description: >
  ...
  build every skill, command, and test for the new plugin.
interactive: false
depends_on:
...
Autonomous skill that reads the spec documents produced by the plugin-spec-generator and produces a complete, step-by-step execution guide. The guide contains ONE FULL PROMPT per skill — never "repeat this pattern." This is the document Claude Code follows to build the plugin skill by skill.
...
1. **Process steps are NEVER vague.** Every step must name:
   - The specific file it reads or writes
   - The specific section or field within that file
   - The specific action performed (read, validate, generate, compute, write)
   - If interactive: the exact question asked and what examples to offer

2. **Steps from the implementation plan are the starting point, not the final form.** Expand each step by:
   - Adding the exact file paths (resolve `[plugin-name]` placeholders)
   - Adding field names from the YAML schema
   - Adding measurable quantities (minimum items, word counts, thresholds)
   - Adding cross-references to other skills' outputs when applicable

3. **Brand data is explicit.** If a skill needs brand data:
   - List the exact section names (identity, audience, voice, colors, typography, visual_identity, logo, content_rules, social_media, assets)
   - Reference the brand-reference.yml schema location in ecosystem-strategy.md

4. **Checkpoints are copied verbatim** from the implementation plan's skill section, then enhanced:
   - If a check says "at least N items" — keep it exactly
   - If a check is vague — make it measurable by adding a quantity or threshold
   - Minimum 2 checks per skill, aim for 3-5
...
3. **Exhaustiveness check:**
   ...
   - Every prompt must be fully self-contained — no cross-references to other prompts
...
- `docs/[plugin-name]-execution-guide.md` — complete build guide with one full prompt per skill
...
These rules are enforced during execution guide generation:
...
3. **Process steps are concrete.** Every step names specific files, sections, and fields. "Process the data" is not a step. "Read `seo-strategy.yml` section `keywords.primary` and filter entries where `search_volume > 100`" is a step.
4. **Checks are measurable.** "Output looks good" is not a check. "At least 5 entries in the `keywords` array, each with `term` (string) and `search_volume` (number > 0) fields populated" is a check.
