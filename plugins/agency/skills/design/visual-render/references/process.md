...
copy_map:        { route → { page_copy_yaml } }  # empty {} if app-copy hasn't run
...
**Optional input (graceful fallback):**

| File | Skill | Fallback |
|------|-------|----------|
| At least 1 page copy YAML | app-copy | Use placeholder copy (see below) |

If page copy files exist in `content/pages/*.yml`, load them into `copy_map`.
If no page copy files exist, set `copy_map = {}` and enable placeholder mode.
This is **not an error** — the design pipeline is self-contained and does not
require `/agency:content` to have run.

**Placeholder copy rules** (used when `copy_map` has no entry for a section):
- Hero headline: `"[Hero headline — generated by /agency:content]"`
- Hero subheading: `"[Subheading — generated by /agency:content]"`
- CTA buttons: `"Get Started"`, `"Learn More"` (generic defaults)
- Feature titles: `"[Feature title]"`
- Feature descriptions: `"[Feature description — 1-2 sentences about this feature]"`
- Testimonial quotes: `"[Customer testimonial — generated by /agency:content]"`
- Generic body text: `"[Body copy — generated by /agency:content]"`

If all required inputs present, save to findings.md:
```markdown
## Step 1: Inputs Loaded
- Tokens: {N} CSS custom properties parsed
- Components: {N} specs loaded ({names})
- Layouts: {N} pages ({routes})
- Content: {N} page copy files (or "placeholder mode — app-copy not yet run")
...
4. For each section in the layout's `sections[]`:
   ...
   c. Get page copy from `copy_map[route]` for this section's slots.
      If `copy_map` has no entry for this route, use placeholder copy
      (see Step 1 placeholder rules). This is expected when app-copy
